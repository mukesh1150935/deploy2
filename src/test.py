...
def evaluate(data, model):
    ...
    """
    with open('data/bleu/candidate.txt', 'w', encoding='utf-8') as f_pred, open('data/bleu/reference.txt', 'w', encoding='utf-8') as f_ref:
        with torch.no_grad():
            for i in range(len(data.dev_en)):
                en_sent = " ".join([data.en_index_dict[w] for w in data.dev_en[i]])
                print("\n" + en_sent)

                pu_sent = " ".join([data.pu_index_dict[w] for w in data.dev_pu[i]])
                print("".join(pu_sent))

                src = torch.from_numpy(np.array(data.dev_en[i])).long().to(DEVICE)
                src = src.unsqueeze(0)
                src_mask = (src != 0).unsqueeze(-2)
                out = greedy_decode(model, src, src_mask, max_len=MAX_LENGTH, start_symbol=data.pu_word_dict["BOS"])

                translation = []
                for j in range(1, out.size(1)):
                    sym = data.pu_index_dict[out[0, j].item()]
                    if sym != 'EOS':
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('save/modelp.pt', map_location=torch.device('cpu')))
